...
The data frame `investr::crystal` gives the growing time and final weight of `r nrow(investr::crystal)` crustals:
...
* Use the `cor()` and `cor.test()` functions to assess the Pearson correlation between these two variables
    ...
    - Interpet the *p*-value in the output
...
* Ideally, we cwant estimates of $\beta_0$ and $\beta_1$ that give us the "best fitting" line
...
* LS estimates
    ...
    - $\widehat{\beta}_1 = \frac{\sum_{i=1}^n\left(X_i - \bar{X}\right)\left(Y_i - \bar{Y}\right)}{\sum_{i=1}^n\left(X_i - \bar{X}\right)^2} = \frac{S_{xy}}{ S_{xx}}$
...
.larger[Fit an SLR model to the crystal weight data using `weight` as the response and `time` as the predictor and intepret the estimated coeficients.]
...
* Does interpretting the intercept make sense in this problem?
...
1. Simulate $Y_i \stackrel{indep.}{\sim} N\left(\mu = 1 + 2X_i, \sigma^2 = 1.2^2\right), \quad i = 1, 2, ... 30$, where $X_i \stackrel{iid}{\sim} U\left(0, 5\right)$

2. Fit an SLR model to the data and estimate the slope and intercept
