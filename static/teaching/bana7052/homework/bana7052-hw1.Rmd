...
Use R or SAS to analyze the given data and answer the following questions. Consider the alumni giving rate as the response variable $Y$ and the percentage of classes with fewer than 20 students as the predictor variable $X$.

a)	Start with exploratory data analysis. Show summary statistics of the response variable and predictor variable. 

b)	What is the nature of variables X and Y? Are there outliers? What is the correlation coefficient? Draw a scatter plot. Any major comments about the data?

c)	Fit a simple linear regression on the data. What is your estimated regression equation? 
...
**Question 2.** A Simulation Study (Simple Linear Regression). Assuming $E(Y|X)= 10 + 5 * X$:

a)	Generate data with $X ~ N\left(\mu = 2, \sigma^2 = 0.12\right)$, sample size $n = 100$, and error term $\epsilon \sim N\left(\mu = 0, \sigma^2 = 0.25\right)$. 
