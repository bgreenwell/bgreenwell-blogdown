...
a) Generate data with $X \sim N\left(\mu = 2, \sigma^2 = 0.12\right)$, sample size $n = 100$, and error term $\epsilon \sim N\left(\mu = 0, \sigma^2 = 0.25\right)$. **Hint:** You can use `rnorm(n = 50, mean = 5, sd = 3)` to simulate $n = 50$ observations from a $N\left(\mu = 5, \sigma = 3\right)$ distribution, but note that `rnorm()` specifies the standard deviation ($\sigma$), rather than the variance ($\sigma^2$), of the normal distribution.
